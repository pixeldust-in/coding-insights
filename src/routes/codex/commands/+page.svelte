...
<div class="p-6">
	{#if data.commands.length === 0}
		<div class="bg-surface border border-border-subtle rounded-xl p-10 card-elevated text-center">
			<p class="text-text-muted text-sm">No custom prompts found</p>
			<p class="text-text-muted text-xs mt-2">Add <code class="text-accent">.md</code> files to <code class="text-accent">~/.codex/prompts/</code> to create custom slash commands</p>
		</div>
	{/if}
