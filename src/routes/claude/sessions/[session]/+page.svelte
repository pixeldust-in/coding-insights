<script lang="ts">
	...
	async function loadMore() {
		...
		const res = await fetch(
			`/api/claude/sessions/${data.sessionId}/messages?offset=${offset}&limit=50`
		);
...
<Header
	breadcrumbs={[
		{ label: 'Sessions', href: '/claude/sessions' },
		{ label: data.firstPrompt?.slice(0, 40) || data.sessionId.slice(0, 8) }
	]}
/>
